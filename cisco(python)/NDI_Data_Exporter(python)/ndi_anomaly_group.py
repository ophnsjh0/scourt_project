...
filename = f'ndi_anomaly_group_{datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")}.csv'
...
def get_anomaly_grouped(insightsGroupName='', fabricName='', startTs='now-15m', endTs='now', count=10000):
    imdata     = []
    offset     = 0
    ...
    while True:
        ...
        url += f'&endDate={end_time}'
        url += f'&sort=-severity'
        url += f'&startDate={start_time}'
        url += f'&siteStatus=online'
        url += f'&aggr=mnemonicTitle'
        url += f'&count={count}'
        url += f'&offset={offset}'
        print(f'\n[{attempt}] Calling... {url}')
...
def csvHeaders():
    main_attr  = [
        'severity','category','subCategory','count','anomalyStr'
    ]
    headers = {}
...
if __name__ == "__main__":
    print('''
=============================================================================
Nexus Dashboard Insights Anomaly Group Exporter (2024/07/25)
...
=============================================================================
    ...
    login = auth()
    data = get_anomaly_grouped(insightsGroupName='default', fabricName=param[0], startTs=param[1], endTs=param[2])
